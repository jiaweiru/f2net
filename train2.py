...
class SEBrain(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        narrow_wavs, lens = batch.narrow_sig
        narrow_feats = self.compute_feats(narrow_wavs)
        ...
        spec_high = self.modules.model(narrow_feats)
        spec_high = torch.expm1(spec_high)
        predict_spec = torch.cat((narrow_feats, spec_high), dim=2)
        ...
        return {"spec": spec_high, "wav": predict_wav}

    def compute_feats(self, wavs, flag=1):
        ...
        if flag == 1:
            ...
            feats = self.hparams.compute_STFT2(wavs)
        feats_mag = sb.processing.features.spectral_magnitude(feats, power=0.5)
        feats_mag = torch.log1p(feats_mag)
        feats_phase = torch.atan2(feats[:, :, :, 1], feats[:, :, :, 0])
        compress = torch.mul(
            torch.unsqueeze(feats_mag, -1),
            torch.cat(
                (
                    torch.unsqueeze(torch.cos(feats_phase), -1),
                    torch.unsqueeze(torch.sin(feats_phase), -1),
                ),
                -1,
            ),
        )
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        clean_spec = self.compute_feats(clean_wavs, 0)
        target = clean_spec[:, :, (self.hparams.n_fft // 4 + 1):, :]

        # Directly compare the spectrogram with the clean targets
        loss = sb.nnet.losses.mse_loss(predictions["spec"],
                                       target,
                                       lens
                                       )
        ...
        if stage != sb.Stage.TRAIN:
            ...
            # for stoi loss, The parameter in the front of the position should be smaller
        ...
        if stage == sb.Stage.TEST:
            clean_wavs, lens = batch.clean_sig
            if not os.path.exists("./test_result2") and clean_wavs.shape[-1] > 16000*5:
                clean_wavs = clean_wavs.to('cpu')
                predictions["wav"] = predictions["wav"].to('cpu')
                os.mkdir("test_result2")
                down = torchaudio.transforms.Resample(16000, 8000)
                for i in range(clean_wavs.shape[0]):
                    sig_down = down(clean_wavs[i])
                    torchaudio.save(f"./test_result2/down{i}.flac", torch.unsqueeze(sig_down, 0), 8000)
                    torchaudio.save(f"./test_result2/clean{i}.flac", torch.unsqueeze(clean_wavs[i], 0), 16000)
                    torchaudio.save(f"./test_result2/infer{i}.flac", torch.unsqueeze(predictions["wav"][i], 0), 16000)
        return loss
...
def dataio_prep(hparams):
    ...
    @sb.utils.data_pipeline.provides("narrow_sig", "clean_sig")
    def audio_pipeline(wav):
        """Load the signal, and pass it and its length to the corruption class.
        This is done on the CPU in the `collate_fn`."""
        clean_sig = sb.dataio.dataio.read_audio(wav)
        clean_sig = clean_sig[:-1] if clean_sig.shape[0] % 2 == 1 else clean_sig

        downsample = torchaudio.transforms.Resample(16000, 8000)
        narrow_sig = downsample(clean_sig)
        return narrow_sig, clean_sig
    ...
    for dataset in ["train", "valid", "test"]:
        datasets[dataset] = sb.dataio.dataset.DynamicItemDataset.from_json(
            ...
            output_keys=["id", "narrow_sig", "clean_sig"],

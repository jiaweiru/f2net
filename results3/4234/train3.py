...
class SEBrain(sb.Brain):
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        if stage == sb.Stage.TEST:
            ...
            if not os.path.exists("./test_result3") and clean_wavs.shape[-1] > 16000*5:
                clean_wavs = clean_wavs.to('cpu')
                noisy_wavs = noisy_wavs.to('cpu')
                predictions["wav"] = predictions["wav"].to('cpu')
                os.mkdir("test_result3")
                down = torchaudio.transforms.Resample(16000, 8000)
                for i in range(clean_wavs.shape[0]):
                    sig_down = down(clean_wavs[i])
                    torchaudio.save(f"./test_result3/down{i}.flac", torch.unsqueeze(sig_down, 0), 8000)
                    torchaudio.save(f"./test_result3/clean{i}.flac", torch.unsqueeze(clean_wavs[i], 0), 16000)
                    torchaudio.save(f"./test_result3/input{i}.flac", torch.unsqueeze(noisy_wavs[i], 0), 16000)
                    torchaudio.save(f"./test_result3/infer{i}.flac", torch.unsqueeze(predictions["wav"][i], 0), 16000)
        return loss
...
def dataio_prep(hparams):
    ...
    def audio_pipeline(wav):
        ...
        wave_down = downsample(clean_sig)
        narrow_spec = hparams["compute_STFT_short"](wave_down.unsqueeze(dim=0))
        noisy_spec = torch.cat((narrow_spec, narrow_spec.flip(dims=[2])), dim=2)
        noisy_spec = noisy_spec[: , :, :-1, :]
        noisy_sig = hparams["compute_ISTFT"](noisy_spec).squeeze(dim=0)
...
if __name__ == "__main__":
    ...
    # output a file inferred by the model
    wav, sr = torchaudio.load("./Male_8k.wav")
    narrow_spec = hparams["compute_STFT_short"](wav)
    noisy_spec = torch.cat((narrow_spec, narrow_spec.flip(dims=[2])), dim=2)
    noisy_spec = noisy_spec[:, :, :-1, :]
    noisy_sig = hparams["compute_ISTFT"](noisy_spec).squeeze(dim=0)
    ...
    bwe = dic["wav"].to('cpu')
    bwe = bwe.squeeze(0)
    sb.dataio.dataio.write_audio("./Male_16k.wav", bwe, 16000)

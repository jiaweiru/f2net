...
class SEBrain(sb.Brain):
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        if stage == sb.Stage.TEST:
            clean_wavs, lens = batch.clean_sig
            if not os.path.exists("./test_result") and clean_wavs.shape[-1] > 16000*5:
                ...
                for i in range(clean_wavs.shape[0]):
                    ...
                    torchaudio.save(f"./test_result/clean{i}.flac", torch.unsqueeze(clean_wavs[i], 0), 16000)
                    torchaudio.save(f"./test_result/infer{i}.flac", torch.unsqueeze(predictions["wav"][i], 0), 16000)
        return loss
